...
class ProductsController < ApplicationController
  ...
  def update
    ...
    add_image_count=params[:product][:big_official_image_attributes].keys.count if params[:product][:big_official_image_attributes].present? 
    add_image_count=params[:product][:picture_attributes].keys.count if params[:product][:picture_attributes].present?

  
    if product.main_image.present?
      product.main_image.update_urls_success? 
      redirect_to ui_indexs_product_image_path(product.id)
      return
    end
    
    if official_images.present?
      official_images.each do |big|
        big.update_urls_success?
      end
  ...
  def update_product_spec_display(sogi_specs,fet_product)
    sogi_specs.each do |fet| 
      spec_id= fet_spec_builder(fet,fet_product.product_type)
      ...
      display.attributes = {:product_id => fet_product.id, 
                            :name => (display.name.blank?)? fet.spec_value.gsub("mm(公厘)","mm") : "#{display.name} x #{fet.spec_value}".gsub("mm(公厘)","mm"), 
                            :spec_id => spec_id
                          }
      display.save!
    end
    ...
    end if fet_product.product_type == 2
  end
